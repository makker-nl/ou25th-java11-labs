...
//        ProductManager pm = new ProductManager(ProductManager.LAN_TAG_CN);
        ...
        pm.parseProduct("D,"+p1Id+",Tea,1.99,0,");
        pm.printProductReport(p1Id);
        pm.parseReview(p1Id+",4,Nice hot cup of tea");
        pm.parseReview(p1Id+",2,Rather weak tea");
        pm.parseReview(p1Id+",4,Fine tea");
        pm.parseReview(p1Id+",4,Good tea");
        pm.parseReview(p1Id+",5,Perfect tea");
        pm.parseReview(p1Id+",3,Just add some lemon");
...
//        pm.createProduct(p3Id, "Cake", BigDecimal.valueOf(3.99), NOT_RATED, LocalDate.now().plusDays(2));
        pm.parseProduct("F,"+p3Id+",Cake,3.99,0,"+LocalDate.now().plusDays(2).toString());
        pm.parseReview(p3Id+",5,Very nice cake");
        pm.parseReview(p3Id+",4,It's good, but I've expected more chocolate");
        pm.parseReview(p3Id+",5,This cake is perfect!");
